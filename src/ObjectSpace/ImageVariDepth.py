...
class Image2DVariDepth(Image2D):
    ...
    def ReceiveAndEmitTowards(self, targets, incidents: RayBatch = None, sampleCount: int = 64,
        ...
        if (incidents is None) or (incidents.IsNoneType()):
            ...
            through = self._CullSelfOcclusionVariDepthAlt(incidents)
    ...
    def _CullSelfOcclusionVariDepthAlt(self, incidents: RayBatch, iterStep: int = 3):
        """
        Cull incident rays against this vari-depth image by finding roots of
        F(t) = ray_z(t) - sampled_image_depth(ray_angle(t)).

        This keeps the same probabilistic opacity behavior as
        _CullSelfOcclusionVariDepth(), but replaces sparse midpoint marching
        with a bracket-and-bisect pass over the depth slab. The working arrays
        scale with ray count times a small number of brackets, not with image
        resolution, so high-resolution depth maps remain practical.
        """

        if incidents is None or incidents.IsNoneType():
            return incidents.Copy() if incidents is not None else RayBatch()

        if (self.zDistance is None) or (self.alphaArray is None):
            return incidents.Copy()

        alpha_eps = 1e-6
        depth_limit = self.zFarLimit * self.zUnitConversion

        alpha_valid = (self.alphaArray > alpha_eps) & (
                bd.abs(self.zDistance) < depth_limit
        ) & bd.isfinite(self.zDistance)
        ...
        dz_eps = 1e-8
        valid_dz = bd.abs(dz) > dz_eps
        safe_dz = bd.where(valid_dz, dz, bd.ones_like(dz))
        ...
        def sample_at(t_param):
            """
            Sample F(t), alpha, and local validity for t_param with shape
            (N, S). Local depth is blended only from finite, in-range, opaque
            taps to avoid transparent/invalid pixels bending the surface.
            """
            p = positions[:, None, :] + t_param[..., None] * directions[:, None, :]
            ...
            # Pixel-center mapping, matching _GeneratePolarPointSources().
            # Clip before flooring so bilinear fractions stay in [0, 1].
            x_img = bd.clip(u * W - 0.5, 0.0, W - 1.0)
            y_img = bd.clip(v * H - 0.5, 0.0, H - 1.0)
            ...
            v00 = (a00 > alpha_eps) & bd.isfinite(z00) & (bd.abs(z00) < depth_limit)
            v10 = (a10 > alpha_eps) & bd.isfinite(z10) & (bd.abs(z10) < depth_limit)
            v01 = (a01 > alpha_eps) & bd.isfinite(z01) & (bd.abs(z01) < depth_limit)
            v11 = (a11 > alpha_eps) & bd.isfinite(z11) & (bd.abs(z11) < depth_limit)

            vw00 = bd.where(v00, w00, bd.zeros_like(w00))
            vw10 = bd.where(v10, w10, bd.zeros_like(w10))
            vw01 = bd.where(v01, w01, bd.zeros_like(w01))
            vw11 = bd.where(v11, w11, bd.zeros_like(w11))

            valid_weight = vw00 + vw10 + vw01 + vw11

            z_sum = vw00 * z00 + vw10 * z10 + vw01 * z01 + vw11 * z11
            z_local = z_sum / bd.maximum(valid_weight, bd.array(1e-12, dtype=PRECISION_TYPE))

            alpha_local = w00 * a00 + w10 * a10 + w01 * a01 + w11 * a11
            alpha_local = bd.clip(alpha_local, 0.0, 1.0)

            local_valid = inside & (valid_weight > 0.0) & (alpha_local > alpha_eps)
            residual = pz - z_local

            return residual, alpha_local, local_valid

        # Degenerate depth field: solve the single z-plane directly.
        if float(z_min) >= float(z_max):
            t_hit = (z_min - oz) / safe_dz
            valid_hit = valid_dz & (t_hit > 0)

            residual, alpha_hit, sample_valid = sample_at(t_hit[:, None])
            alpha_acc = bd.where(
                valid_hit & sample_valid[:, 0],
                alpha_hit[:, 0],
                bd.zeros_like(t_hit),
            )
        ...
        t_enter = (z_min - oz) / safe_dz
        t_exit = (z_max - oz) / safe_dz
        ...
        k = bd.arange(sample_count, dtype=PRECISION_TYPE) / steps
        k = k[None, :]

        t_grid = t0[:, None] + (t1 - t0)[:, None] * k
        t_grid = bd.where(has_seg[:, None], t_grid, bd.zeros_like(t_grid))

        residual, alpha_grid, sample_valid = sample_at(t_grid)

        z_span = bd.abs(z_max - z_min)
        root_tol = bd.maximum(
            bd.array(1e-6, dtype=PRECISION_TYPE),
            z_span * bd.array(1e-9, dtype=PRECISION_TYPE),
        )

        sample_hit = sample_valid & has_seg[:, None] & (bd.abs(residual) <= root_tol)
        alpha_samples = bd.where(sample_hit, alpha_grid, bd.zeros_like(alpha_grid))

        f_left = residual[:, :-1]
        f_right = residual[:, 1:]
        valid_left = sample_valid[:, :-1]
        valid_right = sample_valid[:, 1:]
        ...
        f_b_left = f_left
        f_b_right = f_right
        valid_b_left = valid_left
        valid_b_right = valid_right

        refine_iterations = 8
        for _ in range(refine_iterations):
            t_mid = (b_left + b_right) * 0.5
            f_mid, _alpha_mid, valid_mid = sample_at(t_mid)

            same_side_left = valid_mid & valid_b_left & ((f_b_left * f_mid) > 0.0)
            use_left_side = bracket & same_side_left

            b_left = bd.where(use_left_side, t_mid, b_left)
            f_b_left = bd.where(use_left_side, f_mid, f_b_left)
            valid_b_left = bd.where(use_left_side, valid_mid, valid_b_left)

            use_right_side = bracket & ~same_side_left
            b_right = bd.where(use_right_side, t_mid, b_right)
            f_b_right = bd.where(use_right_side, f_mid, f_b_right)
            valid_b_right = bd.where(use_right_side, valid_mid, valid_b_right)

        t_hit = (b_left + b_right) * 0.5
        f_hit, alpha_hit, valid_hit = sample_at(t_hit)
        bracket_hit = bracket & valid_hit & (bd.abs(f_hit) <= bd.abs(f_b_right - f_b_left) + root_tol)
        alpha_roots = bd.where(bracket_hit, alpha_hit, bd.zeros_like(alpha_hit))

        alpha_all = bd.concatenate([alpha_samples, alpha_roots], axis=1)
        step_T = bd.clip(1.0 - alpha_all, 0.0, 1.0)
        alpha_acc = 1.0 - bd.prod(step_T, axis=1)
        alpha_acc = bd.clip(alpha_acc, 0.0, 1.0)
    ...
    def _GeneratePolarPointSources(self, appendAOV=False):
        ...
        if appendAOV:
            ...
            if alpha_flat is not None:
                ...
                aov_names.append(self.alphaChannelName or "A")

            # depth as second AOV column
            aov_cols.append(depth_flat.reshape(-1, 1))
            aov_names.append(self.depthChannelName or "Z")

            # other AOVs (in the order stored in self.AOVs)
            for name, flat in zip(self.AOVNames, extra_aov_flats):
                aov_cols.append(flat.reshape(-1, 1))
                aov_names.append(name)

            self.pointSourceAOVNames = aov_names
